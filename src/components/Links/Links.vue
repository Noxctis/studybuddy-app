    ...
    <v-dialog v-model="newLinkOpen" width="500">
      <v-card v-on:keyup.enter="addLink()">
        <v-toolbar dark color="primary">
          <v-btn icon dark @click="closeNewLink()">
            <v-icon>mdi-close</v-icon>
          </v-btn>
          <v-toolbar-title>{{ t('link.title') }}</v-toolbar-title>
          <v-spacer />
          <v-toolbar-items>
            <v-btn variant="text" @click="addLink()">
              {{ t('save') }}
            </v-btn>
          </v-toolbar-items>
        </v-toolbar>
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-text-field
                  :label="placeholderLinkName && !newLink.name ? placeholderLinkName : 'Name'"
...
watch(() => newLink.value.url, (url) => {
  ...
  idleTimer = setTimeout(async () => {
    ...
    if (name) placeholderLinkName.value = name
  }, 300)
})
