<template>
  <div class="pomopause">
    ...
    <v-btn class='btn bg-primary pomo-btn pomo-box font-press btn-main-start'
      v-if="showBtnHistory" @click="emit('show-history')">
    ...
    <v-btn class='btn bg-accent pomo-btn pomo-box font-press btn-main-start'
      v-if="showBtnStart"
        @click="start()"
        :disabled="!termsStore.acceptedTermsCheck">
...
const showBtnHistory = computed(() => !pomodoro.going && !pomodoro.settingUp);
const showBtnStart = computed(() => pomodoro.settingUp || !pomodoro.going || (pomodoro.shortPomo && pomodoro.pomodoroStatus.endActual));

function start() {
  termsStore.acceptTerms();
  if (pomodoro.settingUp) {
    pomodoro.startPomodoro();
  } else {
    pomodoro.createPomodoro();
    settingsStore.generalSettings.hideSetup ? pomodoro.startPomodoro() : pomodoro.setup()
  }
}
